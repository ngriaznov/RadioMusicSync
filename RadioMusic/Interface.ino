...
void checkInterface() {
  ...
  channel = map(channel, 0, 1024, 0,
                FILE_COUNT[PLAY_BANK]);  // Highest pot value = 1 above what's
                                         // possible (ie 1023+1) and file count
                                         // is one above the number of the last
                                         // file (zero indexed)
  ...
  if (bankTimer > HOLDTIME) {
    ...
    if (NEXT_CHANNEL >= FILE_COUNT[PLAY_BANK])
      NEXT_CHANNEL = FILE_COUNT[PLAY_BANK] - 1;
    CHAN_CHANGED = true;
    bankTimer = 0;
    meterDisplay = 0;
